...
$$
t_i'=\mathrm{unit}[0]\cdot t_i,\quad y_i'=\mathrm{unit}[2]\cdot y_i
$$
...
$$
\vec {a}(t)=\frac{d\vec {v}}{dt}\quad \mathrm{(중앙차분)}
$$

- 이미 가속도라면 그대로 사용

### 3. 🧮 손상 궤적 적분 (Runge-Kutta 4차)
- 시스템 방정식:

$$
\ddot {x}=\vec {a}(t)-C\cdot \dot {x}-K\cdot x
$$


- 이걸 4차 Runge-Kutta 방식으로 적분하여 $x(t)$ 궤적을 구함

### 4. 📈 손상량 계산
- 각 시점에서 손상량:

$$
D_i=\beta \cdot \| \vec {x}_i\| _2=\beta \cdot \sqrt{x_i^2+y_i^2+z_i^2}
$$
...
[가속도 생성 or 그대로 사용]
        ↓
[2차계 ODE 적분 → 손상 궤적 x(t)]
        ↓
[손상량 D_i = β‖x_i‖ 계산]
        ↓
[최댓값 D_max 추출]
        ↓
[결과 반환: (성공 여부, D_max, 궤적)]
```
