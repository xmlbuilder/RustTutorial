# 🧠 ref 키워드란?
Rust에서 `ref` 는 패턴 매칭이나 구조 분해 시, 값이 아닌 참조를 바인딩하고 싶을 때 사용.  
...
```
- 여기서 `ref name` 을 쓰지 않으면 name은 String을 소유하게 되어 tuple의 `소유권이 이동` 해버림.  
- `ref` 를 쓰면 `&String` 으로 바인딩되어 tuple은 여전히 유효하게 남아 있음.
...
- 특히 `ref`, `ref mut`, `mut` 의 의미가 겹치던 부분을 명확히 분리.
