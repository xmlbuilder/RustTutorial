...
#### 📚 관련 트레잇
Rust에서는 Deref라는 트레잇이 이 동작을 정의합니다. Box<T>, Rc<T>, Arc<T> 같은 스마트 포인터는 이 트레잇을 구현해서 * 연산자를 사용할 수 있게 해줍니다.
...
- Rust는 자동 deref를 지원해서 .len()이나 .push() 같은 메서드 호출 시 *를 생략할 수 있어요.
...
- *r이 먼저 평가되고, 그 결과에 + 2가 적용됩니다.
하지만 다음처럼 괄호 없이 쓰면 Clippy나 컴파일러가 경고할 수 있어요:
```rust
let y = *r + 2 * 3;
```
...
### 🧩 Clippy가 경고하는 경우
Clippy는 다음과 같은 상황에서 경고를 줄 수 있어요:
...
```
이건 Rust가 “아, 이 타입은 Deref를 구현했네. 그럼 내부 값으로 접근해도 되겠지” 하고 알아서 처리해주는 거예요.


### ❌ 자동 deref가 안 되는 경우
하지만 수식, 비교, 복사, 대입 같은 경우에는 자동 deref가 적용되지 않아요.
이럴 땐 명시적으로 *를 써야 합니다.
```rust
let r = &10;
let x = *r + 5; // OK
...
```
이 차이 때문에 “어떤 데선 되는데, 어떤 데선 에러 나네?” 하는 혼란이 생기는 거죠.


## 🧩 왜 Rust는 이렇게 했을까?
Rust는 명확성과 안전성을 중시하는 언어예요. 자동 deref는 편리하지만, 모든 곳에 적용하면 모호한 코드가 생길 수 있어요.  
그래서 Rust는:
- 메서드/필드 → 자동 deref 허용
- 수식/연산 → 명시적 deref 요구
이렇게 의도적으로 구분한 거예요.
...
- Deref 트레잇을 구현하면 *y가 내부의 T를 꺼낼 수 있게 됩니다.
- type Target = T;은 *y가 어떤 타입을 반환할지를 지정합니다.
- fn deref(&self) -> &T는 *y가 내부 값을 참조할 때 호출되는 메서드입니다.

## ✨ 왜 이게 중요한가?
- 이걸 통해 사용자 정의 타입도 참조처럼 동작하게 만들 수 있어요.
...
Box::into_inner(b)는 그 데이터를 꺼내면서 b의 소유권을 가져가므로, b는 더 이상 유효하지 않아요.
