...
즉, Rust가 자동으로 deref를 해주는 거예요. 이걸 deref coercion이라고 부릅니다.
...
- &*a는 Box가 가리키는 실제 객체의 참조
- 결국 자기 타입에 맞는 함수가 실행됨 → 이게 JungHwan님이 말한 “자기 함수 reference”예요!
...
fn main() {
    ...
    println!("{}", (*r).x); // ✅ OK
    // println!("{}", r.x); // ❌ 에러: 자동 deref가 안 됨 (구조체 참조는 명시적 deref 필요)
}
```
