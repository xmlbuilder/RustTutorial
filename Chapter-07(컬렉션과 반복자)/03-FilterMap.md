...
|---------------|------------------------------|-----------------------|------------------------------------|-------------|
|`filter`   |`bool` 반환 조건식 사용      |`Iterator<Item=T>`|요소를 걸러내기만 할 때        |<br/>`filter(|x| x > 10)`           |
|`filter_map`|`Option<T>` 반환 조건 + 변환|`Iterator<Item=U>`|조건 검사와 동시에 값 추출/변환|<br/>`filter_map(|x| Some(x.len()))`|
