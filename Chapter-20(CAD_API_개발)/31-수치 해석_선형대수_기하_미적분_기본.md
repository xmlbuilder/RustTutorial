# 수치 해석 선형대수, 기하, 미적분 기본

아래는 주어진 함수들의 수학적 의미를 수식과 함께 설명하고, 기능별로 정리한 표도 함께 제공합니다.  
이 함수들은 선형대수, 기하, 미적분, 수치해석의 핵심 알고리즘을 구현한 고급 수학 도구입니다.

##  수학적 설명 요약
### 1. 선형 시스템 해법 (LU 분해 기반)
- 문제: $AX=B$
- 해법: $A=LU$ 로 분해 후,
- $LY=B$ (전진 대입)
- $UX=Y$ (후진 대입)
...
$$
a=d₁·d₁, \quad b=d₁·d₂, \quad c=d₂·d₂
$$

$$
e=d₁·(p₂−p₁), \quad f=d₂·(p₂−p₁)
$$
...
- 적응형:
반복적으로 중간점을 추가하며 오차가 기준 이하가 될 때까지 반복

5. 다항식 및 도함수 (Horner 방식)
- 다항식:
...
|-----------------------------|-------------------|------------------------------------------------------------|
| `solve_linear_system`       | AX = B 해법       | $A = LU \Rightarrow LY = B, UX = Y \Rightarrow X$     |
| `solve_linear_system_vec`   | Ax = b 해법       | LU 분해 + 전진/후진 대입                                   |
| `on_intersect_3d_lines_option` | 3D 직선 교차점   | $s, t$ 계산 → $p_1 + s d_1 = p_2 + t d_2$          |
| `closest_points_of_3d_lines` | 3D 직선 최소 거리 | $P(s), Q(t)$ → 두 직선의 최근접점                      |
| `unitized_derivative`       | 단위 벡터 도함수  | (𝒗/‖𝒗‖)'           |
| `on_integrate_simpson`      | 수치 적분         | $\int_a^b f(x)\,dx \approx \frac{b-a}{6}[f(a)+4f(m)+f(b)]$ |
| `on_polynomial_f_df`        | 다항식 및 도함수  | $f(u), f'(u)$ → Horner 방식으로 계산                   |
...
pub fn unitized_derivative(v: &Self, v1: &Self) -> Self {
    ...
    // 원문 코드에 있던 오류 주의: v'·v 가 아니라 v1·v
    let numer =
        Self::scale(v1, s) -
...
```
## 📐 테스트 수식 및 설명 요약
## 1. 선형 시스템 해법 (solve_linear_system, solve_linear_system_vec)
- 문제: $AX=B$
- 해법: $LU$ 분해 후 전진/후진 대입
- 예시:
...
2. 3D 직선 교차점 및 최소 거리 (on_intersect_3d_lines_option, on_closest_points_of_3d_lines)
- 두 직선:
...
- 결과: 두 점 사이 벡터는 두 방향에 수직, 거리 = 최소 거리

3. 단위 벡터 도함수 (unitized_derivative)
- 정의:
...
4. 수치 적분 (on_integrate_simpson_simple)
- 심프슨 공식:
...
5. 다항식 및 도함수 (on_polynomial_f_df)
- 다항식:
...
| 테스트 범주                     | 포함된 테스트 함수들                                                                 |
|--------------------------------|--------------------------------------------------------------------------------------|
| ✅ 선형 시스템 해법             | `test_solve_linear_system_simple`, `test_solve_linear_system_multi_rhs`, `test_solve_linear_system_vec` |
| ✅ 3D 직선 교차 및 최소 거리    | `test_intersect_3d_lines_min_distance`, `test_on_intersect_3d_lines_option`, `test_min_distance_skew_lines`, `test_closest_points_of_3d_lines` |
| ✅ 단위 벡터 도함수             | `test_matches_numeric_derivative`, `test_derivative_is_orthogonal_to_u`, `test_parallel_change_gives_zero`, <br> `test_scale_invariance`, `test_zero_guard_for_tiny_v`, `test_axis_case` |
| ✅ 수치 적분 (심프슨 방식)      | `test_integrate_simpson_sin`, `test_integrate_simpson_poly`, `test_on_integrate_simpson_simple` |
| ✅ 다항식 및 도함수 계산        | `test_polynomial_f_df_basic`, `test_on_polynomial_f_df`                            |
| ✅ 호 분할 계산                 | `test_calculate_arc_segments`                                                      |
| ✅ 정렬 배열 삽입 검증          | `test_insert_value_into_sorted_array`                                              |


## 📊 전체 테스트 기능 요약 표 (확장 버전)

| 테스트 함수 이름                     | 기능 요약                         | 수학적 기대 결과 또는 핵심 수식                                      |
|--------------------------------------|----------------------------------|----------------------------------------------------------------------|
| `test_solve_linear_system_simple`    | 2×2 선형 시스템 해법              | $x = 1.6,\ y = 1.8$                                             |
| `test_solve_linear_system_multi_rhs` | 다중 RHS 선형 시스템 해법         | $X = A^{-1} B$                                                  |
| `test_solve_linear_system_vec`       | 벡터 기반 Ax = b 해법            | $x = \frac{64}{9},\ y = -\frac{29}{9}$                          |
| `test_intersect_3d_lines_min_distance` | 3D 직선 교차점 계산            | $s = 0,\ t = 1$                                                 |
| `test_min_distance_skew_lines`       | 3D 직선 최소 거리점 계산         | 두 점 사이 벡터가 두 방향에 수직                                    |
| `test_closest_points_of_3d_lines`    | 최근접 점 쌍 계산                | $P(s), Q(t)$                                                    |
| `test_matches_numeric_derivative`    | 단위 벡터 도함수 수치 검증       | $u'(0) \approx \frac{u(h) - u(-h)}{2h}$                         |
| `test_derivative_is_orthogonal_to_u` | 도함수 수직성 검증               | $u \cdot u' = 0$                                                |
| `test_parallel_change_gives_zero`    | 평행 변화 시 도함수 0            | $v' \parallel v \Rightarrow u' = 0$                             |
| `test_scale_invariance`              | 스케일 불변성 검증               | $u'(kv, kv') = u'(v, v')$                                       |
| `test_zero_guard_for_tiny_v`         | 경계값 처리                       | $|\vec{v}| \approx 0 \Rightarrow u' = 0$                        |
| `test_axis_case`                     | 축 기준 수동 검산                | $v = (1,0,0),\ v' = (0,1,0) \Rightarrow u' = (0,1,0)$           |
| `test_integrate_simpson_sin`         | 사인 함수 적분                   | $\int_0^{\pi} \sin(x)\, dx = 2$                                 |
| `test_integrate_simpson_poly`        | 다항식 적분                       | $\int_0^3 (3x^2 + 2x + 1)\, dx = 39$                            |
| `test_on_integrate_simpson_simple`   | 일반 적분                         | $\int_0^1 x^2\, dx = \frac{1}{3}$                               |
| `test_polynomial_f_df_basic`         | 다항식 및 도함수 계산            | $f(2) = 0,\ f'(2) = 5$                                          |
| `test_on_polynomial_f_df`            | 다항식 평가                       | $f(1) = 6,\ f'(1) = 8$                                          |
| `test_calculate_arc_segments`        | 호 분할 계산                      | 총 호 길이 = 분할 수 × 분할 길이 (정확도 검증)                      |
...
#[cfg(test)]
mod math2_tests {
    use nurbslib::core::maths::{on_closest_points_of_3d_lines, on_insert_value_into_sorted_array_option,
...
```
```rust
    /// u = v/|v|이면 u·u = 1 ⇒ (u·u)' = 0 ⇒ u'는 u에 수직이어야 한다.
    #[test]
    fn test_derivative_is_orthogonal_to_u() {
        ...
        for (v, v1) in cases {
            ...
            assert!(approx(dot, 0.0, 1e-8), "u·u' = {} (should be ~0)", dot);
        }
    }
```
```rust
    /// v'가 v와 평행이면 방향은 변하지 않는다 ⇒ u' = 0
    #[test]
...
```
```rust
    /// 스케일 불변성: (v, v')에 같은 스케일 k를 곱해도 u(t)는 동일 ⇒ u'도 동일
    #[test]
...
```rust
    /// 간단한 수동 검산 케이스 (축 기준)
    /// v=(1,0,0), v'=(0,1,0): |v|=1, v'·v=0 ⇒ u' = v'/|v| = (0,1,0)
    #[test]
    fn test_axis_case() {
...
```rust
    ...
    fn test_integrate_simpson_poly() {
        // ∫_0^3 (3x^2 + 2x + 1) dx = [x^3 + x^2 + x]_0^3 = 27 + 9 + 3 = 39
...
#[cfg(test)]
mod geometric_tests {
    use nurbslib::core::maths::{on_closest_points_of_3d_lines, on_integrate_simpson_simple,
        on_intersect_3d_lines_option, on_polynomial_f_df, on_solve_linear_system_vec};
    use nurbslib::core::matrix::Matrix;
    use nurbslib::core::prelude::Vector;
```
```rust
