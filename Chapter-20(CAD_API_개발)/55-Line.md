...
| 단계 | 수식 표현                                                                 | 설명                                 |
|------|----------------------------------------------------------------------------|--------------------------------------|
| 1. 중심점 계산 | $\mathbf{c} = \frac{1}{n} \sum_{i=1}^{n} \mathbf{p}_i$             | 점들의 평균값 (centroid)             |
| 2. 산포 행렬 S | 
| 공분산 기반 산포 행렬 (3×3) |
| 3. SVD 분해 | $S = U \Sigma V^T$ → 최대 특이값 방향 선택                         | 주성분 방향 = 직선 방향               |
| 4. 직선 방향 | $\vec{d} = \text{principal eigenvector}$                          | 가장 큰 특이값에 대응하는 방향 벡터  |
| 5. 투영값 계산 | $t_i = (\mathbf{p}_i - \mathbf{c}) \cdot \vec{d}$                 | 각 점을 직선 방향으로 투영한 값      |
| 6. 시작/끝점 | 
| 투영값 범위로부터 직선 양 끝점 계산 |
...
$$
\mathbf{start} = \mathbf{c} + t_{\min} \cdot \vec{d} \\ \quad
\mathbf{end} = \mathbf{c} + t_{\max} \cdot \vec{d}
$$


## 🧠 단계별 설명
### ① 중심점 계산
- 모든 점의 평균을 구해 중심점 $\mathbf{c}$ 를 계산합니다.
- 이는 직선이 통과해야 할 기준점입니다.
### ② 산포 행렬 구성
- 각 점에서 중심점을 뺀 벡터들의 외적을 누적하여 3×3 산포 행렬 S를 만듭니다.
- 이는 PCA(주성분 분석)의 핵심 단계입니다.
### ③ SVD 분해
- 산포 행렬을 SVD로 분해하여 주성분 방향을 찾습니다.
- 가장 큰 특이값에 대응하는 eigenvector가 직선 방향입니다.
### ④ 직선 방향 벡터
- $\vec {d}$ 는 단위 벡터로 정규화된 방향입니다.
- 이 방향을 기준으로 점들을 직선에 투영합니다.
### ⑤ 투영값 계산
- 각 점을 중심점 기준으로 직선 방향에 투영하여 스칼라 값 t_i를 구합니다.
- 이 값의 최소/최대가 직선의 양 끝점 위치를 결정합니다.
### ⑥ 시작/끝점 계산
- $\mathbf{start}=\mathbf{c}+t_{\min }\cdot \vec {d}$
- $\mathbf{end}=\mathbf{c}+t_{\max }\cdot \vec {d}$
- 이 두 점이 최적 적합 직선의 양 끝점입니다.
