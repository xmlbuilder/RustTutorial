# 비균등 분할

이 두 함수는 모두 구간을 나누는 방식에서 **비균등 분할(biased division)** 을 구현합니다.  
수학적으로는 첫 번째는 기하급수적 분할(geometric spacing), 두 번째는 누적분포함수(CDF) 기반 분할입니다.  
아래에 수식과 함께 단계별로 설명.  

## 📐 1. on_generate_biased_divisions: 기하급수적 분할

### 소스 코드
```rust
/// One-sided geometric division
pub fn on_generate_biased_divisions(
    total_length: f64,
    num_div: i32,
    mut r: f64,
    small_at_left: bool,
) -> Vec<f64> {
    let n = num_div.max(0) as usize;
    let mut lt = vec![0.0; n + 1];
    if n == 0 {
        return lt;
    }
    if r <= 0.0 {
        r = 1.0;
    }

    let mut flip = false;
    if r < 1.0 {
        r = 1.0 / r;
        flip = true;
    }
    let left_small = small_at_left ^ flip;

    if (r - 1.0).abs() < 1e-12 {
        let a = total_length / (n as f64);
        for i in 0..=n {
            lt[i] = a * (i as f64);
        }
        return lt;
    }

    let a = total_length / ((r.powi(num_div.max(0)) - 1.0) / (r - 1.0));

    if left_small {
        lt[0] = 0.0;
        for i in 1..=n {
            lt[i] = a * (r.powi(i as i32) - 1.0) / (r - 1.0);
        }
    } else {
        let mut base = vec![0.0; n + 1];
        for i in 1..=n {
            base[i] = a * (r.powi(i as i32) - 1.0) / (r - 1.0);
        }
        for i in 0..=n {
            lt[i] = total_length - base[n - i];
        }
    }
    lt[0] = 0.0;
    lt[n] = total_length;
    lt
}
```

### ✅ 목적
길이 L를 n개의 구간으로 나누되, 한쪽 끝이 더 촘촘하게 분할되도록 합니다.  
비율 r>1일 때, 각 구간 길이는 기하급수적으로 증가합니다.

### 🔢 수식 단계
#### ① 등분일 경우 $(r\approx 1)$

$$
a=\frac{L}{n}\quad \mathrm{(interval\  length)}
$$

$$
x_i=a\cdot i\quad \mathrm{for\  }i=0..n
$$

#### ② 기하급수적 분할일 경우 $(r\neq 1)$
- 총 길이 L는 다음 합으로 표현됩니다:

$$
L=a\cdot \left( 1+r+r^2+\dots +r^{n-1}\right) =a\cdot \frac{r^n-1}{r-1}
$$

- 따라서 첫 구간 길이 a는:

$$
a=\frac{L(r-1)}{r^n-1}
$$

- 누적 위치는:

$$
x_i=a\cdot \frac{r^i-1}{r-1}\quad \mathrm{for\  }i=0..n
$$

- 만약 작은 구간이 오른쪽에 있어야 한다면:

$$
x_i=L-a\cdot \frac{r^{n-i}-1}{r-1}
$$


### ✅ 수학적 의미
- r>1: 왼쪽이 촘촘하고 오른쪽이 넓음
- r<1: 오른쪽이 촘촘하고 왼쪽이 넓음 → 내부적으로 $r=1/r$ 로 변환
- small_at_left 플래그와 r의 크기에 따라 방향이 결정됨

---

## 📐 2. on_generate_smooth_biased_divisions: CDF 기반 부드러운 분할

### 소스 코드
```rust
/// Left/right skewed “smooth” split (Power/Exp CDF)
/// method: 0=Power(t^gamma), 1=Exp((e^{kt}-1)/(e^k-1))
pub fn on_generate_smooth_biased_divisions(
    l: f64,
    n: i32,
    strength: f64,
    small_at_left: bool,
    method: i32,
) -> Vec<f64> {
    let n = n.max(0) as usize;
    let mut x = vec![0.0; n + 1];
    if n == 0 {
        return x;
    }

    let f_power = |t: f64| -> f64 {
        let gamma = if strength <= 0.0 { 1.0 } else { strength };
        t.clamp(0.0, 1.0).powf(gamma)
    };
    let f_exp = |t: f64| -> f64 {
        let t = t.clamp(0.0, 1.0);
        let k = strength;
        if k.abs() < 1e-12 {
            return t;
        }
        let ek = k.exp();
        let ekt = (k * t).exp();
        (ekt - 1.0) / (ek - 1.0)
    };
    let f = |t: f64| -> f64 { if method == 0 { f_power(t) } else { f_exp(t) } };

    for i in 0..=n {
        let t = (i as f64) / (n as f64);
        x[i] = if small_at_left {
            l * f(t)
        } else {
            l * (1.0 - f(1.0 - t))
        };
    }
    x[0] = 0.0;
    x[n] = l;
    x
}
```

### ✅ 목적
길이 L를 n개의 구간으로 나누되, 부드럽게 편향된 분할을 구현합니다.  
Power 함수 또는 지수 함수 기반 누적 분포(CDF)를 사용합니다.

### 🔢 수식 단계
#### ① Power 분할 (method = 0)
- 함수 정의:

$$
f(t)=t^{\gamma }\quad \mathrm{where\  }\gamma >0
$$


- 위치 계산:

$$
x_i=L\cdot f\left( \frac{i}{n}\right) =L\cdot \left( \frac{i}{n}\right) ^{\gamma }
$$


- 반대 방향일 경우:

$$
x_i=L\cdot \left( 1-\left( 1-\frac{i}{n}\right) ^{\gamma }\right)
$$


#### ② Exponential 분할 (method = 1)
- 함수 정의:

$$
f(t)=\frac{e^{kt}-1}{e^k-1}
$$

- 위치 계산:

$$
x_i=L\cdot f\left( \frac{i}{n}\right) \quad \mathrm{or}\quad x_i=L\cdot \left( 1-f\left( 1-\frac{i}{n}\right) \right)
$$

- k>0: 왼쪽이 촘촘
- k<0: 오른쪽이 촘촘
- k=0: 균등 분할

## 📌 요약 비교

| 항목                  | generate_biased_divisions                          | generate_smooth_biased_divisions                                      |
|-----------------------|----------------------------------------------------|------------------------------------------------------------------------|
| 분할 수식             | $x_i = a \cdot \frac{r^i - 1}{r - 1}$          | $x_i = L \cdot f(t)$,<br> $f(t) = t^{\gamma}$ 또는 $\frac{e^{kt} - 1}{e^k - 1}$ |
| 제어 변수             | `small_at_left`, $r$                           | `small_at_left`, `strength`                                           |
| 균등 조건             | $r \approx 1$                                  | $\gamma = 1$, $k = 0$                                          |

---


