## 🔤 UTF‑8 인코딩 규칙 (비트 패턴)

외부 언어(Java, C#, C++, Python 등)와 교환할 때는 UTF-8 변환을 거치는 것이 일반적입니다.  
UTF‑8은 **8비트 단위(바이트)** 로 문자를 표현하며, 가변 길이(1~4바이트) 인코딩을 사용합니다.
문자의 유니코드 코드 포인트 범위에 따라 다음과 같이 비트 패턴이 결정됩니다.

- 1바이트 (U+0000 ~ U+007F)
```
0xxxxxxx
```

- 2바이트 (U+0080 ~ U+07FF)
```
110xxxxx 10xxxxxx
```

- 3바이트 (U+0800 ~ U+FFFF)
```
1110xxxx 10xxxxxx 10xxxxxx
```

- 4바이트 (U+10000 ~ U+10FFFF)
```
11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
```


## Rust UTF-8 길이 판별과 디코딩 코드
아래는 UTF‑8의 비트 패턴을 바탕으로 **첫 바이트로 길이를 판별** 하고, 이어서 **바이트 배열에서 실제 코드 포인트를 디코딩** 하는 Rust 예제입니다.  
각 단계마다 비트 마스크와 시프트 연산을 어떻게 적용하는지 자세히 설명했습니다.

### 개요
- 핵심 규칙: 첫 바이트의 상위 비트 패턴으로 총 바이트 수를 결정합니다.
- 0xxxxxxx → 1바이트
- 110xxxxx → 2바이트
- 1110xxxx → 3바이트
- 11110xxx → 4바이트
- 연속 바이트: 두 번째 바이트부터는 항상 10xxxxxx로 시작합니다.
- 검증 포인트: 연속 바이트 접두어(10), 오버롱(Overlong) 인코딩 금지, 서로게이트 범위 금지, 최대 코드 포인트 U+10FFFF 초과 금지.

### UTF‑8 길이 판별 함수
```rust
/// 첫 바이트로 UTF-8 문자의 총 바이트 수를 판별합니다.
/// 유효하지 않은 리딩 바이트면 Err를 반환합니다.
fn utf8_char_length(first_byte: u8) -> Result<usize, &'static str> {
    if first_byte & 0b1000_0000 == 0 {
        // 0xxxxxxx → 1바이트 (ASCII)
        Ok(1)
    } else if first_byte & 0b1110_0000 == 0b1100_0000 {
        // 110xxxxx → 2바이트
        Ok(2)
    } else if first_byte & 0b1111_0000 == 0b1110_0000 {
        // 1110xxxx → 3바이트
        Ok(3)
    } else if first_byte & 0b1111_1000 == 0b1111_0000 {
        // 11110xxx → 4바이트
        Ok(4)
    } else {
        Err("Invalid UTF-8 leading byte")
    }
}
```

### 동작 설명
- 1바이트 판별: (b & 0b1000_0000) == 0 이면 상위 비트가 0 → ASCII.
- 2바이트 판별: (b & 0b1110_0000) == 0b1100_0000 → 접두어 110.
- 3바이트 판별: (b & 0b1111_0000) == 0b1110_0000 → 접두어 1110.
- 4바이트 판별: (b & 0b1111_1000) == 0b1111_0000 → 접두어 11110.
- 오류 반환: 위 패턴에 해당하지 않으면 UTF‑8 리딩 바이트가 아님.

### 단일 코드 포인트 디코딩 함수
다음 함수는 바이트 슬라이스에서 시작 위치의 문자를 하나 디코딩하여 (char, 사용한 바이트 수)를 반환합니다.  
연속 바이트 접두어 확인, 오버롱 인코딩 금지, 서로게이트 범위 금지, 상한 체크를 포함합니다.
```rust
/// bytes[start..]에서 UTF-8 문자 하나를 디코딩합니다.
/// 성공 시 (문자, 사용한 바이트 수)를 반환합니다.
fn decode_one_utf8(bytes: &[u8], start: usize) -> Result<(char, usize), &'static str> {
    if start >= bytes.len() {
        return Err("Out of range");
    }
    let b0 = bytes[start];
    let len = utf8_char_length(b0)?;

    if start + len > bytes.len() {
        return Err("Incomplete UTF-8 sequence");
    }

    // 연속 바이트 접두어 확인: 10xxxxxx
    for i in 1..len {
        if bytes[start + i] & 0b1100_0000 != 0b1000_0000 {
            return Err("Invalid continuation byte");
        }
    }

    // 코드 포인트 조립
    let codepoint: u32 = match len {
        1 => (b0 & 0b0111_1111) as u32, // 0xxxxxxx → 하위 7비트
        2 => {
            let b1 = bytes[start + 1];
            let x = (b0 & 0b0001_1111) as u32; // 110xxxxx → 하위 5비트
            let y = (b1 & 0b0011_1111) as u32; // 10yyyyyy → 하위 6비트
            (x << 6) | y
        }
        3 => {
            let b1 = bytes[start + 1];
            let b2 = bytes[start + 2];
            let x = (b0 & 0b0000_1111) as u32; // 1110xxxx → 하위 4비트
            let y = (b1 & 0b0011_1111) as u32; // 10yyyyyy
            let z = (b2 & 0b0011_1111) as u32; // 10zzzzzz
            (x << 12) | (y << 6) | z
        }
        4 => {
            let b1 = bytes[start + 1];
            let b2 = bytes[start + 2];
            let b3 = bytes[start + 3];
            let x = (b0 & 0b0000_0111) as u32; // 11110xxx → 하위 3비트
            let y = (b1 & 0b0011_1111) as u32; // 10yyyyyy
            let z = (b2 & 0b0011_1111) as u32; // 10zzzzzz
            let w = (b3 & 0b0011_1111) as u32; // 10wwwwww
            (x << 18) | (y << 12) | (z << 6) | w
        }
        _ => return Err("Unsupported length"),
    };

    // 검증: 오버롱 금지 (최소 바이트로 표현되어야 함)
    match len {
        1 => {
            // 1바이트는 0x00..0x7F만 유효
            if codepoint > 0x7F {
                return Err("Overlong or invalid 1-byte sequence");
            }
        }
        2 => {
            if codepoint < 0x80 || codepoint > 0x7FF {
                return Err("Overlong or out-of-range 2-byte sequence");
            }
        }
        3 => {
            if codepoint < 0x800 || codepoint > 0xFFFF {
                return Err("Overlong or out-of-range 3-byte sequence");
            }
        }
        4 => {
            if codepoint < 0x1_0000 || codepoint > 0x10_FFFF {
                return Err("Overlong or out-of-range 4-byte sequence");
            }
        }
        _ => {}
    }

    // 서로게이트 범위 금지 (UTF-16 전용 영역)
    if (0xD800..=0xDFFF).contains(&codepoint) {
        return Err("Surrogate range is invalid in UTF-8");
    }

    // 상한 초과 금지
    if codepoint > 0x10_FFFF {
        return Err("Code point exceeds Unicode max");
    }

    // Rust char로 변환 (유효한 스칼라 값만 허용)
    match std::char::from_u32(codepoint) {
        Some(ch) => Ok((ch, len)),
        None => Err("Invalid Unicode scalar value"),
    }
}
```

### 디코딩 과정 설명
- 연속 바이트 확인: 각 연속 바이트가 10xxxxxx인지 검사합니다.
- 비트 조립: 리딩 바이트의 하위 비트와 각 연속 바이트의 하위 6비트를 시프트/OR로 합칩니다.
- 2바이트: (x << 6) | y
- 3바이트: (x << 12) | (y << 6) | z
- 4바이트: (x << 18) | (y << 12) | (z << 6) | w
- 오버롱 금지: 예를 들어 U+0041을 2바이트로 표현하면 금지. 각 길이에 맞는 최소/최대 범위를 확인합니다.
- 서로게이트 금지: U+D800..U+DFFF는 UTF‑8에서 금지입니다.
- 최대 상한: U+10FFFF 초과는 금지입니다.
- 스칼라 값 검증: from_u32로 최종 확인합니다.

### 예제 실행
```rust
fn main() {
    let samples: &[&[u8]] = &[
        b"A",                    // U+0041 → 1바이트
        "한".as_bytes(),         // U+D55C → 3바이트 (UTF-8)
        "😀".as_bytes(),         // U+1F600 → 4바이트
    ];

    for s in samples {
        let mut i = 0;
        println!("Bytes: {:?}", s);
        while i < s.len() {
            match decode_one_utf8(s, i) {
                Ok((ch, used)) => {
                    println!("  char='{}' U+{:04X} used={} bytes", ch, ch as u32, used);
                    i += used;
                }
                Err(e) => {
                    println!("  error at offset {}: {}", i, e);
                    break;
                }
            }
        }
        println!();
    }

    // 길이만 판별해보기
    for &b in &[0x41u8, 0xC2u8, 0xE3u8, 0xF0u8] {
        println!("Byte 0x{:X}: len={:?}", b, utf8_char_length(b));
    }
}
```

### 예상 출력
```
Bytes: [65]
  char='A' U+0041 used=1 bytes

Bytes: [237, 149, 156]
  char='한' U+D55C used=3 bytes

Bytes: [240, 159, 152, 128]
  char='😀' U+1F600 used=4 bytes

Byte 0x41: len=Ok(1)
Byte 0xC2: len=Ok(2)
Byte 0xE3: len=Ok(3)
Byte 0xF0: len=Ok(4)
```

### 구현 시 주의할 점
- 연속 바이트 확인: 모든 연속 바이트는 반드시 10xxxxxx여야 합니다.
- 오버롱 인코딩 금지: 가능한 최단 길이로 인코딩되어야 하며, 범위를 벗어나면 오류로 처리합니다.
- 서로게이트 범위 금지: U+D800..U+DFFF는 UTF‑8에서 유효하지 않습니다.
- 에러 처리: 네트워크/파일 입력 등 신뢰할 수 없는 데이터에서는 Result 기반 에러 처리가 필수입니다.
- 성능 고려: 대량 파싱 시에는 루프 전개와 분기 최소화 등을 고려하고, 표준 라이브러리나 검증된 파서 사용도 좋습니다.

---
## 소스 코드
```rust
pub fn on_decode_one_utf8(bytes: &[u8], start: usize) -> Result<(char, usize), &'static str> {
    if start >= bytes.len() {
        return Err("Out of range");
    }
    let b0 = bytes[start];
    let len = on_utf8_char_length(b0)?;

    if start + len > bytes.len() {
        return Err("Incomplete UTF-8 sequence");
    }

    // 연속 바이트 접두어 확인: 10xxxxxx
    for i in 1..len {
        if bytes[start + i] & 0b1100_0000 != 0b1000_0000 {
            return Err("Invalid continuation byte");
        }
    }

    // 코드 포인트 조립
    let codepoint: u32 = match len {
        1 => (b0 & 0b0111_1111) as u32, // 0xxxxxxx → 하위 7비트
        2 => {
            let b1 = bytes[start + 1];
            let x = (b0 & 0b0001_1111) as u32; // 110xxxxx → 하위 5비트
            let y = (b1 & 0b0011_1111) as u32; // 10yyyyyy → 하위 6비트
            (x << 6) | y
        }
        3 => {
            let b1 = bytes[start + 1];
            let b2 = bytes[start + 2];
            let x = (b0 & 0b0000_1111) as u32; // 1110xxxx → 하위 4비트
            let y = (b1 & 0b0011_1111) as u32; // 10yyyyyy
            let z = (b2 & 0b0011_1111) as u32; // 10zzzzzz
            (x << 12) | (y << 6) | z
        }
        4 => {
            let b1 = bytes[start + 1];
            let b2 = bytes[start + 2];
            let b3 = bytes[start + 3];
            let x = (b0 & 0b0000_0111) as u32; // 11110xxx → 하위 3비트
            let y = (b1 & 0b0011_1111) as u32; // 10yyyyyy
            let z = (b2 & 0b0011_1111) as u32; // 10zzzzzz
            let w = (b3 & 0b0011_1111) as u32; // 10wwwwww
            (x << 18) | (y << 12) | (z << 6) | w
        }
        _ => return Err("Unsupported length"),
    };

    // 검증: 오버롱 금지 (최소 바이트로 표현되어야 함)
    match len {
        1 => {
            // 1바이트는 0x00..0x7F만 유효
            if codepoint > 0x7F {
                return Err("Overlong or invalid 1-byte sequence");
            }
        }
        2 => {
            if codepoint < 0x80 || codepoint > 0x7FF {
                return Err("Overlong or out-of-range 2-byte sequence");
            }
        }
        3 => {
            if codepoint < 0x800 || codepoint > 0xFFFF {
                return Err("Overlong or out-of-range 3-byte sequence");
            }
        }
        4 => {
            if codepoint < 0x1_0000 || codepoint > 0x10_FFFF {
                return Err("Overlong or out-of-range 4-byte sequence");
            }
        }
        _ => {}
    }

    // 서로게이트 범위 금지 (UTF-16 전용 영역)
    if (0xD800..=0xDFFF).contains(&codepoint) {
        return Err("Surrogate range is invalid in UTF-8");
    }

    // 상한 초과 금지
    if codepoint > 0x10_FFFF {
        return Err("Code point exceeds Unicode max");
    }

    // Rust char 로 변환 (유효한 스칼라 값만 허용)
    match std::char::from_u32(codepoint) {
        Some(ch) => Ok((ch, len)),
        None => Err("Invalid Unicode scalar value"),
    }
}
```
```rust
pub fn on_utf8_char_length(first_byte: u8) -> Result<usize, &'static str> {
    if first_byte & 0b1000_0000 == 0 {
        // 0xxxxxxx → 1바이트 (ASCII)
        Ok(1)
    } else if first_byte & 0b1110_0000 == 0b1100_0000 {
        // 110xxxxx → 2바이트
        Ok(2)
    } else if first_byte & 0b1111_0000 == 0b1110_0000 {
        // 1110xxxx → 3바이트
        Ok(3)
    } else if first_byte & 0b1111_1000 == 0b1111_0000 {
        // 11110xxx → 4바이트
        Ok(4)
    } else {
        Err("Invalid UTF-8 leading byte")
    }
}
```
```rust
pub fn on_is_invalid_uf8(byte: u8) -> bool {
    on_utf8_char_length(byte).is_ok()
}
```
```rust
/// String → UTF-8 바이트 배열
pub fn on_string_to_utf8_bytes(s: &str) -> Vec<u8> {
    s.as_bytes().to_vec()
}

/// UTF-8 바이트 배열 → String
pub fn on_utf8_bytes_to_string(bytes: &[u8]) -> Result<String, std::string::FromUtf8Error> {
    String::from_utf8(bytes.to_vec())
}
```

## 테스트 코드
```rust
#[cfg(test)]
mod tests {
    use nurbslib::core::string_utils::{on_decode_one_utf8, on_is_invalid_uf8, on_string_to_utf8_bytes, on_utf8_bytes_to_string, on_utf8_char_length};
    use super::*; // 같은 모듈에 정의된 함수 사용

    #[test]
    fn test_utf8_char_length() {
        // 'A' → 0xxxxxxx → 1바이트
        assert_eq!(on_utf8_char_length(0x41).unwrap(), 1);

        // 'Â' → 110xxxxx → 2바이트
        assert_eq!(on_utf8_char_length(0xC2).unwrap(), 2);

        // 'ã' → 1110xxxx → 3바이트
        assert_eq!(on_utf8_char_length(0xE3).unwrap(), 3);

        // 😀 → 11110xxx → 4바이트
        assert_eq!(on_utf8_char_length(0xF0).unwrap(), 4);
    }
```
```rust
    #[test]
    fn test_decode_one_utf8_ascii() {
        let bytes = b"A"; // U+0041
        let (ch, used) = on_decode_one_utf8(bytes, 0).unwrap();
        assert_eq!(ch, 'A');
        assert_eq!(used, 1);
    }
```
```rust
    #[test]
    fn test_decode_one_utf8_korean() {
        let bytes = "한".as_bytes(); // U+D55C
        let (ch, used) = on_decode_one_utf8(bytes, 0).unwrap();
        assert_eq!(ch, '한');
        assert_eq!(used, 3);
    }
```
```rust
    #[test]
    fn test_decode_one_utf8_emoji() {
        let bytes = "😀".as_bytes(); // U+1F600
        let (ch, used) = on_decode_one_utf8(bytes, 0).unwrap();
        assert_eq!(ch, '😀');
        assert_eq!(used, 4);
    }

    #[test]
    fn test_invalid_byte() {
        // 0xFF는 유효한 UTF-8 리딩 바이트가 아님
        assert!(!on_is_invalid_uf8(0xFF));
    }
```
```rust
    #[test]
    fn main() {
        let original = String::from("안녕하세요 😀");

        // String → UTF-8 바이트 배열
        let utf8_bytes = on_string_to_utf8_bytes(&original);
        println!("UTF-8 bytes: {:?}", utf8_bytes);

        // UTF-8 바이트 배열 → String
        match on_utf8_bytes_to_string(&utf8_bytes) {
            Ok(restored) => println!("Restored String: {}", restored),
            Err(e) => println!("Error decoding UTF-8: {}", e),
        }
    }
}
```



