...
#### Rust 쪽 (지금)
- 상속 → 트레이트(행위/계약)    
    HasAABB 트레이트(예: fn aabb(&self) -> BoundingBox)를 만족하는 타입이면 무엇이든 트리에 넣을 수 있음.  
...
```
- 트리는 T: HasAABB 받음. = **넣을 수 있는 것** 의 정의가 **타입 계보** 가 아니라 **가능한 행위** 가 됨.
...
- 복잡한 동작이나 규칙을 숨겨서 몰래 처리하지 말고, 함수 이름이나 타입으로 드러내서 코드만 봐도  
  어떤 정책이 적용되는지 명확하게 알 수 있도록 함.
...
### 5) 점진 이행 팁
- 어댑터 타입
    struct OnObjectAdapter<'a>(&'a ON_Object);가 HasAABB 구현해서 기존 ON_Object도 트리에 넣을 수 있게 가교를 만듦.  
...
### RayQuery 분리
- IntersectsRay 트레이트 신설. 트리는 HasAABB 알게 하고, 레이 정확도/충돌 세부는 외부 시스템이 맡게.
...
- 처음부터 **전부 바꾸기** 보다 경계(Adaptor) 를 만들어 섬→대륙 순으로 넓힘.
