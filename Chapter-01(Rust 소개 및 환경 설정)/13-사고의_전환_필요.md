...
#### 제네릭 함수 하나 작성:
```rust
fn overlaps<T: SpatialObject, U: SpatialObject>(a: &T, b: &U) -> bool {
...
### 3) 소유/공유/식별
#### 과제 C: “삭제/업데이트”
- SpatialTree<T: SpatialObject>를 만들고 삽입/삭제/업데이트 구현.
...
```
- `SpatialTree<T: SpatialObject + IntersectsRay>` 로 제약 확장.
...
- unsafe 구간은 어댑터 내부로 가둬두고, 외부엔 SpatialObject/IntersectsRay만 노출.
...
impl SpatialObject for Mesh   { fn aabb(&self) -> &BoundingBox { &self.bbox } }
impl SpatialObject for Sphere { fn aabb(&self) -> &BoundingBox { &self.bbox } }

// 제네릭 함수: 상속 없이 조합
fn overlaps<T: SpatialObject, U: HasAabb>(a: &T, b: &U) -> bool {
    a.aabb().intersects(b.aabb())
}
...
```rust
trait SpatialObject { fn aabb(&self) -> &BoundingBox; }

struct SpatialTree<T: SpatialObject> {
    nodes: Vec<Node>,
...
- 제네릭로 시작하면 lifetime/타입 전파가 폭발해요.
  초판은:
